...
//	@HystrixCommand
	public List<CatalogItem> getCatalog(@PathVariable("userId") String userId) {
		...
		Movie[] movies = restTemplate.getForObject("http://spring-profile-movie-info/movies/user/" + userId,
				Movie[].class);
		...
		return Arrays.asList(movies).stream().map(movieObj -> {
			...
			Rating rating = restTemplate
					.getForObject("http://SPRING-PROFILE-MOVIE-RATINGS/ratings/" + movieObj.getMovieId(), Rating.class);
